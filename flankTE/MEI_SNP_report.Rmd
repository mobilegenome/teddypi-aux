...
test_types <- vector()
test_types["SINE"] = "/home/fritjof/teddypi/data/2017-03-09/MEI_SNP/asian/"
...
ggsave(paste("~/teddypi/results//MS/MEI_SNP/insratio",dset,"pdf", sep="."), device="pdf", width=8.27, height=8.27, units="in")
```
...
head(df)
levels(as.factor(df$clade_support))
dfm <- melt(dfs, id.vars=c("dset", "clade_support", "flank"))
insSNP_summary <- summarySE(dfm,groupvars=c("dset","variable", "clade_support", "flank"), measurevar="value")
View(insSNP_summary)
p <- plot_context_alt_topologies_MS(insSNP_summary, "SINE")
